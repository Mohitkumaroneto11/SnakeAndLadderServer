...
export class KnifeHitXFac extends XFacAbstract {
    private user: User
    private game: Game
    ...
    private async playGame() {
        try {
            ...
            if (this.turnCount % 8 == 0) {
                this.game?.log('8th chance come score = 8');
                score = 5
            } 
            this.log('Xfac playTurn=>', score, noOfCut, SCORE_LIST, this.speed, this.speed*1000);
            await this.updateScore(score, this.game.ID);
            setTimeout(this.playGame.bind(this), this.speed * 1000);
        } catch (err) {
    ...
    public async startGame() {
        this.turnCount = 0;
        setTimeout(this.playGame.bind(this), 8000);
        // setInterval(this.checkOpponentStatus.bind(this), 5000);
        setTimeout(this.ai.bind(this), 18500);
    }
    ...
    private ai() {
        try {
            ...
            if (this.speed < MIN_SPEED) {
                this.log('Speed found min')
                this.speed = MIN_SPEED
            } else if (this.speed > MAX_SPEED) {
                this.log('Speed found max')
        ...
        setTimeout(this.ai.bind(this), 500);
